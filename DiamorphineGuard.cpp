...
VOID SendToLogs(std::string log, LPCWSTR  webhook) {
	...
	std::string title = ("DiaGuard Logs");
	std::string body = ("DiaGuard");
	std::string color = ("16711680"); // Decimal color
	std::string request_body = ("{\"username\": \"") + body + ("\",\"content\": null,\"embeds\": [{\"title\": \"") + title + ("\",\"description\": \"") + log + ("\",\"footer\": {\"text\": \"") + body + ("\"},\"color\": ") + color + (" }], \"attachments\": []}");
